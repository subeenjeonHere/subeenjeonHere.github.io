...
> 김영한님 강의에서 배운 걸 적용해 봤다. 구현력을 늘려(?)보고, 공부하고 싶어서 여러 형태로 파라미터를 받아왔다. 그리고 게시판과  관련 요청을 처리하는 컨트롤러 내에서 PathVariable, RequestParam, ModelAttribyute, Model 을 사용했는데, 지금 코드를 볼 땐 사실은 일관성이 없어 보인다.. 이게 좋은 습관인건지 모르겠다.
...
sequenceDiagram
    ...
    사용자->>컨트롤러: 삭제 버튼 클릭 (게시글 ID 전달)
    컨트롤러->>DAO: 게시글 ID를 DAO 메소드에 전달
    DAO->>DAO: 받은 게시글 ID를 이용해 데이터베이스에서 게시글 삭제 쿼리 실행
    DAO-->>컨트롤러: 삭제 성공 표시
    Controller->>User: '게시글 삭제가 완료되었습니다.' 메시지 전달 및 게시판 페이지로 리다이렉트
...
![image](https://github.com/subeenjeonHere/subeenjeonHere.github.io/assets/145312273/216658b0-9e74-4562-951a-e51700e702a2)
...
```mermaid
sequenceDiagram
사용자->>컨트롤러: "수정" 버튼 클릭 (Post 요청 전송)
컨트롤러->>DAO: 게시글의 ID와 수정된 내용 전송
DAO->>데이터베이스: 해당 게시글의 데이터 수정 쿼리 실행
데이터베이스-->>DAO: 데이터 수정 완료
DAO-->>컨트롤러: 데이터 수정 완료
컨트롤러-->>사용자: "게시글 수정이 완료되었습니다." 메시지 전송
