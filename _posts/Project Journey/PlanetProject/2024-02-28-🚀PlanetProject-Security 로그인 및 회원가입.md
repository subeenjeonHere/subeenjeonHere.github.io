...
* [☻ Security 로그인 및 로그아웃](#-security-로그인-및-로그아웃)
  ...
  * [☺︎ 로그인](#-로그인)
    ...
    * [UserDetails 인터페이스란?](#userdetails-인터페이스란)
    * [사용한 이유는](#사용한-이유는)
* [☻ 세션 관리](#-세션-관리)
    * [동작 방식](#동작-방식-1)
  * [Session Flags](#session-flags)
* [✏️ 궁금했던 것](#궁금했던-것)
    * [세션 플래그가 XSS를 어떻게 막아주는 것인지](#세션-플래그가-xss를-어떻게-막아주는-것인지)
    * [세션 플래그가 세션, 쿠키를 HTTPS로만 전송되도록 하는 방법?](#세션-플래그가-세션-쿠키를-https로만-전송되도록-하는-방법)
    * [세션 매니지먼트와, 플래그의 차이점?](#세션-매니지먼트와-플래그의-차이점)
* [☻ 현재 로그인 한 회원을 추적하기](#-현재-로그인-한-회원을-추적하기)
    * [동작 방식](#동작-방식-2)
* [📌 발생했던 에러 정리](#발생했던-에러-정리)
    * [1. 올바른 정보로 로그인 해도 에러가 발생한다.](#1-올바른-정보로-로그인-해도-에러가-발생한다)
