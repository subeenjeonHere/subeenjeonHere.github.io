...
1. 최댓값을 찾고, 나머지 2가지 원소의 합이 최댓값 보다 크다면 삼각형을 만들 수 있다.
    1. 굳이 자료구조를 사용해야 하는가
    2. 매개변수로 배열이 주어졌다. 배열에서 최댓값을 찾자. ArrayList, Collections를 사용해서 Max를 추출하고, 나머지 값들의 합을 구한다.
    3. 그리고 합이 Max보다 크면 삼각형을 만들 수 있고, 작으면 만들 수 없다.
...
```java
정확성  테스트
테스트 1 〉	통과 (0.08ms, 65.8MB)
테스트 2 〉	통과 (0.05ms, 73.4MB)
테스트 3 〉	통과 (0.09ms, 71.8MB)
테스트 4 〉	통과 (0.06ms, 77MB)
테스트 5 〉	통과 (0.05ms, 77.1MB)
테스트 6 〉	통과 (0.06ms, 75.5MB)
테스트 7 〉	통과 (0.06ms, 67.6MB)
테스트 8 〉	통과 (0.05ms, 68.1MB)
테스트 9 〉	통과 (0.06ms, 73.2MB)
```

> 두 번째 풀이
>

다른 사람 풀이 참고해서 풀어봤는데, 애초에 내림차순 정렬했으면 더 간결했을 듯하다.
...
```java
테스트 1 〉	통과 (0.44ms, 72.8MB)
테스트 2 〉	통과 (0.32ms, 74MB)
테스트 3 〉	통과 (0.44ms, 75.2MB)
테스트 4 〉	통과 (0.40ms, 76.9MB)
테스트 5 〉	통과 (0.35ms, 86.7MB)
테스트 6 〉	통과 (0.35ms, 79.6MB)
테스트 7 〉	통과 (0.41ms, 69.9MB)
테스트 8 〉	통과 (0.36ms, 78.6MB)
테스트 9 〉	통과 (0.36ms, 71.3MB)
```

### 왜 코드가 몇 배로 간결한 두 번째가 실행 속도가 느린가

아마 `Arrays.sort()` 메소드 사용에 따른 오버헤드 때문일 가능성일 수 있다. 배열을 정렬하는 것은 두 숫자를 더하는 것보다 계산상 더 복잡한 작업이기 때문.
